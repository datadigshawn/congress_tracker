...
# ── 標的說明資料庫（SQLite）──────────────────────────────────────
# ════════════════════════════════════════════════════════════════
_DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ticker_info.db")
...
def load_senate_trades(days: int, today: str) -> pd.DataFrame:
    """從 CapitolTrades 載入參議院 PTR 交易資料。"""
    since = datetime.now() - timedelta(days=days)
    rows: list[dict] = []
    page = 1
    max_pages = 100
...
with st.sidebar:
    ...
    country = st.radio("資料來源", ["🇺🇸 美國國會", "🇹🇼 台灣立委", "🏦 機構 13F"], horizontal=True)
...
# ════════════════════════════════════════════════════════════════
    ...
    # ── 立委統計 ──
    _tw_leg_count = dff_tw["立委"].nunique()
    _tw_co_count  = dff_tw["公司"].nunique()
    st.markdown(
        f"共 **{_tw_leg_count}** 位立委持有 **{_tw_co_count}** 家公司股票"
    )
    ...
    if _tw_selected != "（請選擇公司）":
        ...
        _tw_legs   = dff_tw[dff_tw["公司"] == _tw_selected]["立委"].nunique()
        _tw_shares = dff_tw[dff_tw["公司"] == _tw_selected]["股數"].sum()
        st.info(
            f"**{_tw_selected}**　｜　板塊：{_tw_sector}　｜　"
            f"共 {_tw_legs} 位立委持有、合計 {_tw_shares:,.0f} 股\n\n"
            f"{_tw_desc or '暫無說明'}",
            icon="📊",
        )
        # 編輯區
